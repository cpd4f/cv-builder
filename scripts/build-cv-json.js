...
async function fetchCvItems(recordId) {
  ...
  do {
    const params = new URLSearchParams({
      filterByFormula: `AND({Publish}=TRUE(), FIND("${recordId}", ARRAYJOIN({Parent-CV})))`,
      pageSize: "100",
    ...
    offset = data.offset || "";
    logDebug("Fetched CV Items page", { count: (data.records || []).length, hasMore: Boolean(offset), filterField: "Parent-CV" });
  } while (offset);
